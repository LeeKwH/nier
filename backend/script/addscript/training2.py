...
seq_info = list(input_tmp['seqnum'].values())

dir_source = nowpath+'/.user/'+args.user_name+'/.data/'+input_dataset+'/'+input_dataset+'.csv'
dir_input = nowpath+'/.user/'+args.user_name+'/.model/'+args.model_name+'/.input/'
...
list_inps = []
for idx,inp in enumerate(df_input):
    tensor_input = []
    for i in range(0, len(output)):
        if first_cols[idx] in list_lr_2d:
            dateperiod = inp[output.index[i]-timedelta(days=seq_info[idx]):output.index[i]].values
            tensor_input.append(dateperiod)
        else: 
            datepoint = inp.loc[output.index[i]-timedelta(days=seq_info[idx])].values
            tensor_input.append(datepoint)
