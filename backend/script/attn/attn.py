...
data['유해남조류세포수'] = data['유해남조류세포수'].interpolate(method='linear', limit=7, limit_direction='backward')
...
# make target column
data['유해남조류세포수_Y'] = data['유해남조류세포수'].shift(-Outshift)
...
X = model_df.iloc[:,:-1].values.reshape(model_df.shape[0], input_t_step, x_features)
...
test_yhat = pd.DataFrame(y_pred,columns=['유해남조류세포수'])
test_y = pd.DataFrame(y_test,columns=['유해남조류세포수'])
