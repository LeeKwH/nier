...
export default function Preprocessing(){
    ...
    useEffect(()=>{makeChartfromProc([]); const defaultChecks = Array(region.length).fill(false);setCheckBox(defaultChecks); setIsimport(false); 
        // 오류 function 처리
        let tmpfuncs = [...funcs];
    ...
    const LoadData = ()=>{
        const std = dayjs(startDate).startOf("D").format();
        const end = dayjs(endDate).startOf("D").format();
    ...
    const inforeload = ()=>{
        if(rows.length!==0){fetch('/api/dataframe/info',{
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
                ...
                <div className="val-table-wrap">
                    <TableContainer component={Paper} sx={{height:'100%',overflow:'auto'}}>
                        <Table size="small" stickyHeader sx={{width:'100%'}} aria-label="val-table">
                            ...
                            <TableBody>
                                {region.map((data,idx)=> <TableRow hover><TableCell>x{idx}</TableCell>{data.map((d)=><TableCell align="center">{d}</TableCell>)}</TableRow>)}
                            </TableBody>
                        </Table>
                    </TableContainer>
                </div>
